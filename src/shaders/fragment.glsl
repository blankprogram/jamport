...
void main() {
    ...
    float final = mix(brightness, 1.0f - brightness, uInvert);

    float offset = 0.1f;

    if(uChromatic && uRainbow) {
        vec3 r = hsv2rgb(vec3(hueAt(uv + vec2(offset, 0.0f)), 1.0f, uValue));
        vec3 g = hsv2rgb(vec3(hueAt(uv), 1.0f, uValue));
        vec3 b = hsv2rgb(vec3(hueAt(uv - vec2(offset, 0.0f)), 1.0f, uValue));
        fragColor = vec4(mix(r.r, 1.0f - r.r, uInvert), mix(g.g, 1.0f - g.g, uInvert), mix(b.b, 1.0f - b.b, uInvert), 1.0f);
    } else if(uChromatic) {
        float r = hueAt(uv + vec2(offset, 0.0f)) * uValue;
        float g = hueAt(uv) * uValue;
        float b = hueAt(uv - vec2(offset, 0.0f)) * uValue;
        fragColor = vec4(mix(r, 1.0f - r, uInvert), mix(g, 1.0f - g, uInvert), mix(b, 1.0f - b, uInvert), 1.0f);
    } else if(uRainbow) {
        vec3 rgb = hsv2rgb(vec3(hue, 1.0f, uValue));
        fragColor = vec4(mix(rgb, vec3(1.0f) - rgb, uInvert), 1.0f);
    } else {
