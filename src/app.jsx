...
const App = () => {
    ...
    setTimeout(() => {
      ...
      const type = () => {
        ...
        if (index < fullText.length) {
          ...
          if (char === "\n") {
            setIsPaused(true);
            setTimeout(() => { setIsPaused(false); type(); }, 1000);
  ...
  // Cursor blink
  useEffect(() => {
    if (!isPaused && !typingDone) return setShowCursor(true);
  ...
  return (
    <div id="scroll-container" className="relative h-screen w-screen overflow-y-auto scroll-smooth snap-y snap-mandatory" style={{ scrollSnapType: "y mandatory" }}>
      <ShaderBackground />
      ...
      <div id="content" className="relative z-10">
        ...
        <section id="more-section" className="min-h-screen bg-[#181a1b] text-white flex items-center justify-center px-10 py-20 snap-start" style={{ scrollSnapAlign: "start" }}>
          <div className="max-w-3xl text-center">
            ...
            <p className="text-lg text-gray-300">I'm passionate about building clean, scalable web applications.</p>
          </div>
        </section>
      </div>
    </div>
